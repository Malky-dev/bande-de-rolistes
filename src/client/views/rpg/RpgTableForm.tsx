...
export default function RpgTableForm(props: Props): ReactElement | null {
  const initial = useMemo<RpgTableFormValues>(() => {
    return props.mode === 'edit' ? valuesFromTable(props.table) : defaultCreateValues()
  }, [props])
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        if (mounted) setAvailableStatuses([...RPG_TABLE_STATUSES])
      } finally {
  ...
  const submit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (props.mode === 'create') {
        ...
        const nextStatus = values.status ?? props.table.status
    ...
    <section className="rpg-create">
      ...
      <form onSubmit={(e) => void submit(e)} className="form">
        ...
        <div className="formField--grow">
            <label htmlFor="rpg-eventDate">Choisir la date et l'heure de la table dans le calendrier</label>
            <DatePicker
                id="rpg-eventDate"
                selected={values.eventDate}
                onChange={(d: Date | null) => setValues((v) => ({ ...v, eventDate: d }))}
                showTimeSelect
                timeFormat="HH:mm"
                timeIntervals={30}
        ...
        <div>
            <label htmlFor="rpg-maxPlayers">Nombre de joueurs maximum - les suivants seront placés sur la liste d'attente</label>
            <select
                id="rpg-maxPlayers"
                value={String(values.maxPlayers)}
                onChange={(e) => setValues((v) => ({ ...v, maxPlayers: Number(e.target.value) }))}
                disabled={isSubmitting}
                required
            >
        ...
        <div>
          <label htmlFor="rpg-location">Choisir l'endroit ou se déroulera la table</label>
          <select
            id="rpg-location"
        ...
        <div>
          <label htmlFor="rpg-comments">Commentaire - Pour informer les joueurs sur la table</label>
          <textarea
            id="rpg-comments"
        ...
        <div>
            ...
            <div className="radio-group">
                {(availableStatuses.length > 0 ? availableStatuses : [...RPG_TABLE_STATUSES]).map((s) => (
        ...
        <div className="form-actions">
          <button type="button" className="btn-secondary" onClick={props.onBack} disabled={isSubmitting}>
            Retour
          </button>
