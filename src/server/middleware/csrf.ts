...
export function verifyCsrf<
  P extends ParamsAny = ParamsAny,
  B extends object = object,
>(): RequestHandler<P, ApiError, B> {
  return (req, res, next) => {
    try {
      ...
      if (!token || typeof token !== "string") {
        res.status(403).json({
          code: "FORBIDDEN",
          message:
            "Token CSRF manquant (header x-csrf-token ou body csrfToken).",
        });
        return;
      }
      ...
      if (!tokens.verify(secret, token)) {
        res
          .status(403)
          .json({ code: "FORBIDDEN", message: "Token CSRF invalide" });
        return;
      }
...
export function generateCsrfToken(
  req: ReqWithCookieHeader,
  res: ResWithCookieSetter,
): string {
