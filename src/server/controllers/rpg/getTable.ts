...
const controllerGetTable: RequestHandler<
  GetTableParams,
  GetTableResponse,
  Record<string, never>
> = async (req, res): Promise<void> => {
  try {
    ...
    if (eventID === null) {
      res
        .status(400)
        .json({ code: "BAD_REQUEST", message: "eventID invalide" });
      return;
    }
    ...
    if (
      typeof dmUserIDUnknown !== "number" ||
      typeof dmNicknameUnknown !== "string"
    ) {
      res.status(500).json({ code: "ERROR", message: "MJ invalide" });
      return;
    }
    ...
    if (
      typeof locationUnknown !== "string" ||
      typeof gameUnknown !== "string"
    ) {
      res.status(500).json({ code: "ERROR", message: "Table invalide" });
      return;
    }
    ...
    if (
      typeof maxPlayersUnknown !== "number" ||
      !Number.isFinite(maxPlayersUnknown)
    ) {
    ...
    const comments =
      typeof commentsUnknown === "string"
        ? commentsUnknown
        : commentsUnknown === null
          ? null
          : null;
    ...
    for (const signup of signups) {
      ...
      if (
        typeof suUserIDUnknown !== "number" ||
        typeof suNicknameUnknown !== "string"
      ) {
        continue;
      }
