...
const controllerCreateTable: RequestHandler<Record<string, never>, CreateResponse, CreateBody> = async (
  req,
  res
): Promise<void> => {
  try {
    ...
    const { eventDate, dungeonMasterUserID, location, game, comments, maxPlayers } = req.body
    ...
    if (!dm) {
      res.status(400).json({ code: 'BAD_REQUEST', message: 'Le MJ doit avoir un rôle 1, 2 ou 3' })
      return
    }
    ...
    const mp =
      typeof maxPlayers === 'number' && Number.isFinite(maxPlayers) ? Math.floor(maxPlayers) : 10
