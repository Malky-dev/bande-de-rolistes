...
const controllerLogin: RequestHandler<Record<string, never>, LoginResponse, LoginBody> = async (
  req,
  res
): Promise<void> => {
  try {
    ...
    if (typeof userRaw !== 'object' || userRaw === null || !isUserWithPassword(userRaw)) {
      const payload: ApiError = {
        code: 'NOT_FOUND',
