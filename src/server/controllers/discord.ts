...
invariant(
  ...
  "Variables d'environnement Discord manquantes (DISCORD_CLIENT_SECRET)",
);
invariant(
  DISCORD_REDIRECT_URI,
  "Variables d'environnement Discord manquantes (DISCORD_REDIRECT_URI)",
);
invariant(
  FRONTEND_URL,
  "Variables d'environnement Discord manquantes (FRONTEND_URL)",
);
...
export const controllerDiscordInit: RequestHandler = (req, res): void => {
  try {
    const state =
      typeof req.query.state === "string" ? req.query.state : "default";
...
// ---------------------------
export const controllerDiscordCallback: RequestHandler = async (
  req,
  res,
): Promise<void> => {
  try {
    ...
    if (!user) {
      const nickname =
        discordUser.username ?? `Discord_${discordUser.id.slice(0, 8)}`;
