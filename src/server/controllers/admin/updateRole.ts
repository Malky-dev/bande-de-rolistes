...
// ---------------------------
const controllerAdminUpdateRole: RequestHandler<
  Params,
  UpdateRoleResponse,
  Body
> = async (req, res): Promise<void> => {
  try {
    ...
    if (req.user?.userID === user.userID) {
      res
        .status(403)
        .json({ code: "FORBIDDEN", message: "Cannot modify your own role" });
      return;
    }
    ...
    const updatedUser = await User.findByPk(userID, {
      attributes: ["userID", "nickname", "email", "roleID", "isVerified"],
      include: [
        {
          model: Role,
          as: "role",
